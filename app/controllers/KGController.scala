...
import helpers.ResponseHelper.{filterContentTypeAndLengthFromHeaders, flattenHeaders, getContentType}
import helpers.{IDMHelper, NexusHelper}
...
class KGController @Inject()(cc: ControllerComponents)(implicit ec: ExecutionContext, ws: WSClient, config: Configuration)
  ...
  def createSchema(organization: String, domain: String, entityType: String, version: String): Action[AnyContent] = Action.async { implicit request =>
    ...
    NexusHelper.createSchema(entityType, s"$organization/$domain", version, token, organization).map {
      response =>
