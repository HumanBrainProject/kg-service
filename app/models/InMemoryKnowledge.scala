...
class InMemoryKnowledge(space: String) {
  ...
  def loadManualSchemaList(token: String)(implicit ws: WSClient, ec: ExecutionContext, config: Configuration) = {
    val schemaUrl = s"${config.get[String]("nexus.endpoint")}/v0/schemas/$space"
